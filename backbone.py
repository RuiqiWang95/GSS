...
class ResNet12(nn.Module):
    ...
    def __init__(self, emb_size, block=ResNet12Block, cifar_flag=False):
        ...
        in_f = cfg[3]*2*2 if cifar_flag else cfg[3]*6*6
        self.HW = 2 if cifar_flag else 6
        self.fc = nn.Sequential(nn.Linear(in_features=in_f,
                                          out_features=self.emb_size,
                                          bias=True),
                                nn.BatchNorm1d(self.emb_size))

        # self.AttLayer = nn.Sequential(nn.Conv2d(2,2,1),
        #                               nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #                               nn.Conv2d(2,1,1),
        #                               nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.last_norm = nn.BatchNorm1d(self.emb_size)
    ...
    def forward(self, x):
        ...
        # C_self = x.transpose(1,3).flatten(start_dim=1, end_dim=-2)
        # C_global= x.transpose(1,3).flatten(end_dim=-2)
        # Att_self = torch.bmm(C_self.transpose(1,2), C_self)
        # Att_global = torch.mm(C_global.transpose(0,1), C_global).unsqueeze(0).repeat(Att_self.size(0),1,1)
        # Att_in = torch.stack([Att_self, Att_global], dim=1)
        # Att_out = self.AttLayer(Att_in).squeeze(1).softmax(dim=1)
        # gathered = torch.bmm(C_self, Att_out).view(x.size(0), self.HW, self.HW, x.size(1)).transpose(1,3)
        # x += gathered

        x = x.view(x.size(0), -1)
        x = self.fc(x)
        CC = torch.mm(x.transpose(0,1), x) / x.size(0)
        att = CC.softmax(dim=0)
        x = x-x.mm(att)
        x = self.last_norm(x)
...
if __name__ == '__main__':
    import torch
    net = ResNet12(128, cifar_flag=True)
